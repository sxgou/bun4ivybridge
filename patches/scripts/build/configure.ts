 ...
 */
/** Minimal glob for *.ts patterns — replaces globSync for bootstrap compat. */
function simpleGlob(pattern: string, dir: string): string[] {
  const prefix = pattern.includes("/") ? pattern.split("/")[0] + "/" : "";
  const suffix = pattern.includes("/") ? pattern.split("/")[1] : pattern;
  const baseDir = prefix ? resolve(dir, prefix) : dir;
  try {
    return readdirSync(baseDir)
      .filter(e => {
        if (suffix.startsWith("*.")) return e.endsWith(suffix.slice(1));
        return e === suffix;
      })
      .map(e => join(prefix, e));
  } catch { return []; }
...
function configureInputs(cwd: string): string[] {
  ...
  const scripts = simpleGlob("*.ts", buildDir)
    .filter(f => !excluded.has(f))
    .map(f => resolve(buildDir, f));
  const deps = simpleGlob("deps/*.ts", buildDir).map(f => resolve(buildDir, f));
